...
from scripts.ingest.load_remote_csv import DataIngestionError, load_remote_csv, validate_csv
...
class TestLoadRemoteCSV(unittest.TestCase):
    ...
    def test_load_remote_csv_success(self, mock_get):
        ...
        # Execute
        result = load_remote_csv(url=self.test_url, output_path=self.output_path, max_retries=1)
    ...
    def test_load_remote_csv_timeout(self, mock_get):
        ...
        with self.assertRaises(DataIngestionError) as context:
            load_remote_csv(url=self.test_url, output_path=self.output_path, max_retries=1)
