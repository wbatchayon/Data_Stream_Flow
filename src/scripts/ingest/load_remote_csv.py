    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise DataIngestionError(f"Download failed after {max_retries} attempts: Timeout") from e
                
        except requests.exceptions.HTTPError as e:
            logger.error(f"HTTP error on attempt {attempt}: {e}")
            if attempt == max_retries:
                raise DataIngestionError(f"Download failed after {max_retries} attempts: {e}") from e
