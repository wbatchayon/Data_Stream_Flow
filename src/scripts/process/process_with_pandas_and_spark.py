...
def process_with_pandas(
    input_path: str = "/tmp/remote_data.csv", output_path: str = "/tmp/pandas_processed.csv"
) -> str:
    """
...
def process_with_pandas_and_spark(
    ...
    pandas_result = process_with_pandas(input_path=input_csv, output_path=pandas_output)

    # Step 2: Spark processing
    spark_result = process_with_spark(input_path=pandas_result, output_path=spark_output)

    logger.info("Complete data processing pipeline finished")

    return {"pandas_output": pandas_result, "spark_output": spark_result}
